  ...
  post_json "/api/orders" "$(jq -n \
    ...
    '{
      ...
      product: {
        ...
        quantity: 1,
        notes: "Refund path demo placeholder"
      },
      quote: $quote
    }'
  )"
...
print_step "5. Refund request"
refund_request_response="$(
  post_json "/api/orders/${order_id}/refund-request" "$(jq -n \
...
assert_jq "$refund_request_response" '.order.status == "REFUND_PENDING"' "refund request did not move order to REFUND_PENDING"

print_step "6. Refund simulated escrow"
refund_response="$(
  post_json "/api/orders/${order_id}/refund" "$(jq -n \
    --arg buyerUserId "$BUYER_USER_ID" \
    --arg buyerAddress "$BUYER_ADDRESS" \
    --arg buyerPublicKey "$BUYER_PUBLIC_KEY" \
    --arg intermediaryUserId "$INTERMEDIARY_USER_ID" \
    ...
    '{
      ...
      simulatedRefundTxid: "dev-refund-txid",
      networkFeeXec: 10
    }'
  )"
)"
...
print_step "7. Get final order"
final_order_response="$(get_json "/api/orders/${order_id}")"
print_response "$final_order_response"
assert_jq "$final_order_response" '.order.status == "REFUNDED"' "final order status was not REFUNDED"

printf '\nStatus: %s\n' "$(jq -r '.order.status' <<<"$final_order_response")"
printf 'Refund path completed successfully\n'
